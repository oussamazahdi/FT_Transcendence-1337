...
export default function initSocketManager(io) {
	io.on("connection", socket => {
		...
		socket.on("paddle-move", (playerData)=> handlePaddleMove(socket, io, playerData))
		socket.on("update-data", (playerData)=> handleUpdateData(socket, io, playerData))
		socket.on("join-game", playerData => handleJoin(socket, io, playerData));
