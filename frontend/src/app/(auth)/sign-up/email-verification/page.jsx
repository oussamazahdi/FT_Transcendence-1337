    ...
    <div className="min-h-screen flex justify-center items-center">
      <div className="flex flex-row justify-between bg-[#1A1A1A]/75 w-[800px] h-[480px] rounded-4xl">
        <div className="flex flex-col justify-center items-center w-[400px] mx-2 space-y-2">
          ...
          <h2 className="text-[#A6A6A6] text-sm px-6 text-center">We've sent a 6-digit code to <span className="text-white">You@Exemple.com</span></h2>
          ...
          <div className="flex flex-col items-center text-gray-500 text-sm">
            ...
            <button  onClick={handleResend} disabled={!canResend || isResending} className="text-white hover:underline font-medium">
              {isResending ? "Sending..." : canResend ? "Resend Code" : `Resend code in ${timer}s`}
            </button>
          </div>
            {error && <p className="text-red-500 text-xs text-center">{error}</p>}
          <button onClick={handleSubmite} disabled={isVerifying || otp.join("").length < 6} className=" w-81 mt-1 px-4 py-2 bg-[#0F2C34] text-white text-xs rounded hover:bg-[#245664] disabled:bg-gray-500 transition">
          {isVerifying ? "Verifying..." : "Verify"}
          </button>
          <button onClick={() => router.replace("/sign-up/email-verification/selecte-image")} className="text-white text-sm cursor-pointer">
            Not now
          </button>
        </div>
