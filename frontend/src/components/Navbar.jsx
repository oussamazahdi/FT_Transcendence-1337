  ...
  const navBareMap = navItems.map((item) => {
    ...
    return (
      <Link
        key={item.href}
        href={item.href}
        className={`mx-2 text-[#BEBEBE] border border-[#BEBEBE] hover:text-white hover:bg-[#000000]/40 focus:bg-[#000000]/40 rounded-full cursor-pointer py-2 px-4 
				${isActive ? "bg-[#000000]/65 text-white font-medium"  : ""}`}
      >
        {item.label}
      </Link>
    );
